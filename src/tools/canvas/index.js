...
class MCanvas {
  ...
  getTemplateImage(index) {
    ...
    return item.pngImg + Util.constant.THUMB600;
  }
  ...
  getPhotoImage(pageIndex, imageIndex) {
    ...
    if (undefined != currentPhoto) {
      ...
      if (EMPTY == photo.key || photo.width == 0 || undefined == photo.key) {
        ...
        return encodeURI(Util.constant.DOMAIN + photo.key + Util.constant.THUMB600 + '/rotate/' + photo.degree);
      }
    }
    ...
    if (!editable) {
      ...
      if (undefined != currentPhoto) {
        ...
        if (EMPTY == photo.key || photo.width == 0 || photo.key == undefined) { //暂无图片,显示默认图片
          ...
          let url = encodeURI(Util.constant.DOMAIN + photo.key + Util.constant.THUMB600 + '/rotate/' + photo.degree),
